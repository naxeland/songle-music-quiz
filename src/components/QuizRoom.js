...
import {
  buildOnlineCatalog,
  ONLINE_DEFAULT_PLAYLIST_ID,
  ONLINE_PLAYLIST_OPTIONS,
} from "@/lib/onlineCatalog";
import {
...
const ONLINE_NAME_STORAGE_KEY = "songle-online-name";
...
const MODE_CONFIG = {
  ...
  online: {
    ...
    invitePath: "/online-quiz",
    title: "Online Quiz",
    playlistOptions: ONLINE_PLAYLIST_OPTIONS,
    defaultPlaylistId: ONLINE_DEFAULT_PLAYLIST_ID,
  },
};
...
export default function QuizRoom({ mode = "spotify", initialName, initialRoomId }) {
  ...
  const [playlistId, setPlaylistId] = useState(config.defaultPlaylistId);
  ...
  useEffect(() => {
    if (isOnline) {
      if (typeof window === "undefined") return;
      const storedName = window.localStorage.getItem(ONLINE_NAME_STORAGE_KEY);
      if (storedName) {
  ...
  const readySummary = session
    ...
    : "";
  const inviteLink = roomId ? `${getOrigin()}${config.invitePath}?room=${roomId}` : "";
  ...
  const playlistVoteOptions = useMemo(
    () => playlistOptions.filter((playlist) => playlist.id !== playlistId),
    [playlistId]
  );
  ...
  const joinRoom = () => {
    ...
    router.replace(`${config.invitePath}?room=${encodeURIComponent(nextRoomId)}`, undefined, {
      shallow: true,
    });
  };
  ...
  const loadPlaylistCatalog = async (nextPlaylistId) => {
    if (isOnline) {
      return await buildOnlineCatalog(nextPlaylistId);
    }
  ...
  const fetchCatalog = async () => {
    if (!playlistId || (!isOnline && !token)) return;
  ...
  const setReadyForRound = () => {
    ...
    if (isOnline) previewPlayerRef.current?.unlock();
  ...
  const logout = () => {
    if (isOnline) {
      if (typeof window !== "undefined") {
        window.localStorage.removeItem(ONLINE_NAME_STORAGE_KEY);
      }
  ...
  if (!isLoggedIn) {
              ...
              <CardContent>
                <form
                  ...
                  onSubmit={(event) => {
                    ...
                    if (typeof window !== "undefined") {
                      window.localStorage.setItem(ONLINE_NAME_STORAGE_KEY, trimmedName);
                    }
                ...
                <p className="mt-4 text-center text-xs text-muted-foreground">
                  No login needed. Online mode plays 30-second preview clips, so anyone can join.
                </p>
              </CardContent>
            </Card>
          </section>
    ...
    return (
      <SiteShell>
        <section className="grid min-h-[calc(100vh-145px)] place-items-center px-6 py-10 sm:px-10">
          <Login next={router.asPath || config.invitePath} />
        </section>
      </SiteShell>
    );
  }
